...
def login():
    if request.method == 'POST':
        ...
        # Redireciona para a página de login realizado com o nome do usuário inserido
        return redirect(url_for('login_realizado', username=username))
    return render_template('login.html')

@app.route('/login_realizado/<username>')
def login_realizado(username):
    # Obtenha a data e hora local
    current_time = datetime.now().strftime("%B %d, %Y %I:%M %p")
    # Renderize a página de login realizado com os dados do acesso
    return render_template('login_realizado.html', username=username, current_time=current_time)
